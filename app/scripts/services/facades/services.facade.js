...
angular.module('hexaaApp.services.facades').factory('ServicesFacade', ['$http', '$rootScope', '$q', 'HexaaService', 'InvitationsFacade', 'baseUIAddr', 'baseAddr', 'ResourceEntity', 'EntitlementpacksFacade', 'SercviceFactory', function ($http, $rootScope, $q, HexaaService, InvitationsFacade, baseUIAddr, baseAddr, ResourceEntity, EntitlementpacksFacade, SercviceFactory) {
    ...
    var Proxy = {
        ...
        getEntitlementpacks: function (sid, pager) {
            ...
            request.success(function (data, status, headers, config) {
                ...
                angular.forEach(data.items, function (entitlementpack) {
                    entpacks.push(EntitlementpacksFacade.new(entitlementpack));
                });
                deferred.resolve(wrapResponse({
                    items: entpacks,
                    item_number: data.item_number
            ...
            request
                .success(function (data, status, header, config) {
                    ...
                    angular.forEach(data.items, function (invitation) {
                        result.push(InvitationsFacade.new(invitation));
                    });
                    deferred.resolve(wrapResponse({
                        items: result,
                        item_number: data.item_number
        ...
        regenerateHookKey: function(service) {
            return HexaaService.regenerateServiceHookKey(service.id);
        }
    };
    return Proxy;
}]);
