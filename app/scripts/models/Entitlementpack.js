...
function EntitlementpackFactory($injector) {
    ...
    function save() {
        var ServicesProxy = $injector.get("ServicesProxy");
        if (this.id === -1 || this.id === undefined) {
            return ServicesProxy.addEntitlementpack(this.service.id, this);
        }
        else {
            return ServicesProxy.updateEntitlementpack(this);
        }
    }
    ...
    function getToken() {
        var proxy = $injector.get("EntitlementpacksProxy");
