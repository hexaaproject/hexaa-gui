...
(function () {
    ...
    angular.module('hexaaApp.components.profile.controllers.pages')
        ...
        ['$scope', '$translate', 'PrincipalProxy', 'ServicesProxy', 'ConsentsProxy', '$q', 'dialogService', 'pageTitleService',
            function ($scope, $translate, PrincipalProxy, ServicesProxy, ConsentsProxy, $q, dialogService, pageTitleService) {
                 ...
                 */
                function activate() {
                    PrincipalProxy.getRelatedServices($scope.pager)
                ...
                function load(service) {
                    $q.all([ServicesProxy.getAttributeSpecifications(service.id), PrincipalProxy.getServiceConsent(service.id)])
                ...
                function save(service) {
                    ...
                    if (consent.id === undefined)
                        ConsentsProxy.addConsent(consent)
                            .success(onAddConsentSuccess)
                            .error(onAddConsentError);
                    else
                        ConsentsProxy.updateConsent(consent)
