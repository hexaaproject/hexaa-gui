...
(function () {
    ...
    angular.module('hexaaApp.components.organizations.controllers.pages')
        ...
        ['$scope', 'OrganizationsFacade', 'PrincipalFacade', '$cookies', '$translate', 'events', 'dialogService', 'pageTitleService',
            function ($scope, OrganizationsFacade, PrincipalFacade, $cookies, $translate, events, dialogService, pageTitleService) {
                ...
                function onOrganizationSave(event, selectedOrganization) {
                    if (($scope.organization.id != -1) && (selectedOrganization )) {
                        OrganizationsFacade.updateManagers($scope.organization)
                ...
                function onOrganizationsSelectionChanged(event, selectedOrganization) {
                    ...
                    if (
                        ...
                        OrganizationsFacade.getMembers(selectedOrganization)
                            .then(onPrincipalsLoaded)
                            .catch(onPrincipalLoadError);

                        OrganizationsFacade.getManagers(selectedOrganization)
