...
( function () {
    ...
    angular.module('hexaaApp.components.organizations.controllers.pages')
        ...
        ['$scope',  'OrganizationsFacade',  '$translate', 'events',
        '$modal', 'dialogService', 'pageTitleService', 'profileService',
        function ($scope, OrganizationsFacade, $translate, events,
                  $modal, dialogService, pageTitleService, profileService) {
            ...
            function propose(principal) {
                OrganizationsFacade.addManager(vm.organization.id, principal.id)
            ...
            function onGetOrganizationSuccess(data)
            {
                ...
                {
                    OrganizationsFacade.getPrincipals(data.data.id)
            ...
            function onRemovePrincipalConfirmed(principal) {
                return function (answer) {
                    if (answer == "yes") {
                        OrganizationsFacade.removeMember(vm.organization.id, principal.id)
            ...
            function onRevokePrincipalConfirmed(principal) {
                return function (answer) {
                    if (answer == "yes") {
                        OrganizationsFacade.removeManager(vm.organization.id, principal.id)
