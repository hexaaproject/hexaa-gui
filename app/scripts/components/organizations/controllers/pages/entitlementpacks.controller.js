...
(function () {
    ...
    angular.module('hexaaApp.components.organizations.controllers.pages')
        ...
        ['$scope', 'EntitlementpacksFacade', 'OrganizationsFacade', '$translate', 'events', 'dialogService', 'pageTitleService',
            function ($scope, EntitlementpacksFacade, OrganizationsFacade, $translate, events, dialogService, pageTitleService) {
                ...
                function link(token) {
                    if (token != null) {
                        OrganizationsFacade.linkPrivateEntitlementpack($scope.organization, token)
                ...
                function getEntitlementpacksOfOrganization(organization) {
                    //query service
                    OrganizationsFacade.getEntitlementpacks(organization.id)
                ...
                function onDeleteEntitlementpackConfirmed(entitlementpack) {
                    return function (answer) {
                        if (answer === 'yes') {
                            OrganizationsFacade.removeEntitlementpack($scope.organization.id, entitlementpack.id)
                ...
                function onLinkPrivateEntitlementSuccess(data) {
                    ...
                    dialogService.confirm($translate.instant(namespace + "msg.entitlementpackAttachToRoleConfirmTitle"),
                        $translate.instant(namespace + "msg.entitlementpackAttachToRoleConfirm"))
                        .then(onAttachToRoleClosed);
                }

                function onAttachToRoleClosed(answer) {
                    if (answer == 'yes') {
                        $scope.navigate("#/organizations/roles");
                    }
                }
