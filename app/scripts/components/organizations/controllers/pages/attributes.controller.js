...
(function () {
    ...
    angular.module('hexaaApp.components.organizations.controllers.pages')
        ...
        ['$scope', 'dialogService', '$translate', 'AttributeSpecificationsFacade', 'PrincipalFacade', 'events', 'OrganizationsFacade', 'pageTitleService', '$modal',
        function ($scope, dialogService, $translate, AttributeSpecificationsFacade, PrincipalFacade, events, OrganizationsFacade, pageTitleService, $modal) {
            ...
            function organizationsSelectionChanged(event, selectedOrganization) {
                if (selectedOrganization) {
                    if ($scope.profile.isManagerOfOrganization(selectedOrganization)) {
                        ...
                        OrganizationsFacade.getAttributeSpecifications(selectedOrganization)
            ...
            function saveAttribute(attrspec, attribute) {
                if (attribute.id === -1) {
                    ...
                    //persist
                    OrganizationsFacade.addAttributeValueOrganization(attribute)
                        .then(onAttributeValueOrganizationSaved(attrspec, attribute))
                        .catch(onAttributeValueOrganizationSaveError);
                }
                else {
                    //update existing
                    OrganizationsFacade.updateAttributeValueOrganization(attribute)
            ...
            function load(attrspec) {
                ...
                OrganizationsFacade.getAttributeValueOrganizations(attrspec)
                    .then(onGetAttributeValueOrganizationsSuccess(attrspec))
                    .catch(onGetAttributeValueOrganizationsError(attrspec));

                AttributeSpecificationsFacade.getSupportingServices(attrspec)
            ...
            function newAttribute(attrspec, attributeToEdit) {
                ...
                var modalInstance = $modal.open({
                    templateUrl: 'views/components/organizations/modals/attribute-editor-dialog.html',
            ...
            function removeAttribute(attrspec, attribute) {
                OrganizationsFacade.deleteAttributeValueOrganization(attribute)
