...
(function () {
    ...
    angular.module('hexaaApp.components.services.controllers.pages')
        ...
        ['$scope', 'ServicesFacade', '$translate', 'events', 'dialogService', 'EntitlementpacksFacade', 'ResourceEntity', 'pageTitleService', '$modal',
        function ($scope, ServicesFacade, $translate, events, dialogService, EntitlementpacksFacade, ResourceEntity, pageTitleService, $modal) {
            ...
            function onItemPerPageChanged() {
                if (vm.service.id !== -1)
                {
                    ServicesFacade.getEntitlements(vm.service.id, vm.pager)
            ...
            function editEntitlement(entitlementToEdit) {
                ...
                var modalInstance = $modal.open({
                    templateUrl: 'views/components/services/modals/new-entitlement-dialog.html',
            ...
            function save(entitlement) {
                if (vm.service.id != -1) {
                    ...
                    if (entitlement.id == -1) {
                        ServicesFacade.addEntitlement(vm.service.id, entitlement)
                            .then(onAddEntitlementSuccess(entitlement)).
                            catch(onAddEntitlementError(entitlement));
                    }
                    else {
                        //update
                        ServicesFacade.updateEntitlement(entitlement)
            ...
            function onServicesSelectionChanged(event, selectedService) {
                if (selectedService ) {
                    ...
                    ServicesFacade.getEntitlements(selectedService, vm.pager)
                        .then(onGetEntitlementsSuccess);
                    //Get entitlementprefix
                    EntitlementpacksFacade.getEntitlementPrefix()
            ...
            function onDeleteEntitlementpackDialogClosed(entitlement) {
                return function (answer) {
                    if (answer == 'yes') {
                        ServicesFacade.deleteEntitlement(entitlement.id).
