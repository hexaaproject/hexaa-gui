...
(function () {
    ...
    angular.module('hexaaApp.components.services.controllers.pages').controller('ServiceEntitlementpacksCtrl',
        ['$scope', '$window', '$timeout', '$cookies', '$q', '$rootScope', '$translate', 'events', 'EntitlementpacksFacade', 'ServicesFacade', '$modal', 'dialogService', 'pageTitleService',
            function ($scope, $window, $timeout, $cookies, $q, $rootScope, $translate, events, EntitlementpacksFacade, ServicesFacade, $modal, dialogService, pageTitleService) {
                ...
                function editEntitlementpack(entitlementpack) {
                    var modalInstance = $modal.open({
                        templateUrl: 'views/components/services/modals/new-entitlementpack-dialog.html',
                        ...
                        resolve: {
                            ...
                            targetEntitlementpack: function () {
                                if (entitlementpack) {
                                    ...
                                    var brandNew = EntitlementpacksFacade.new({
                                        type: 'private',
                                        id: -1,
                                        entitlements: [],
                ...
                function generateToken(entitlementpack) {
                    EntitlementpacksFacade.generateToken(entitlementpack.id)
                ...
                function refreshEntitlementpacks() {
                    if ($scope.service.id !== -1) {
                        ServicesFacade.getEntitlementpacks($scope.service.id, $scope.pager)
                ...
                function onServicesSelectionChanged(event, selectedService) {
                    if ($scope.selectedService) {
                        ...
                        ServicesFacade.getEntitlements(selectedService)
