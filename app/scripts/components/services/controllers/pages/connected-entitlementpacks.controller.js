...
(function () {
    ...
    angular.module('hexaaApp.components.services.controllers.pages')
        ...
        ['$scope', 'ServicesFacade', 'OrganizationsFacade',  '$translate', 'events', '$modal', 'dialogService', 'pageTitleService',
            function ($scope,ServicesFacade, OrganizationsFacade, $translate, events, $modal, dialogService, pageTitleService) {
        ...
        function showDetails(id) {
            var modalInstance = $modal.open({
                templateUrl: 'views/components/organizations/modals/organization-details-dialog.html',
        ...
        function onServicesSelectionChanged(event, selectedService) {
            ...
            if (selectedService ) {
                ...
                ServicesFacade.getLinkedOrganizations(selectedService)
        ...
        function acceptConnection(connection) {
            ServicesFacade.acceptConnectionRequest(connection.organization_id, connection.entitlement_pack_id)
        ...
        function onDeleteConnectionDialogClosed(connection) {
            return function (answer) {
                if (answer == 'yes') {
                    OrganizationsFacade.deleteEntitlementpack(connection.organization_id, connection.entitlement_pack_id)
